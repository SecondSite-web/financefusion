...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    try {
        ...
        $nonceTest = $dashNonce->verifyNonce($_POST['nonce'], 'delete-group-form');
        if ($nonceTest === false) {
        ...
        if ($test === false) {
            throw new Exception('This Group does not Exists');
        }
        $result = $dashAuth->removeGroup($_POST['groupName']);
        if ($result === false) {
            throw new Exception('Unable to save, please refresh and try again');
        }

        $responseArray = array('type' => 'success', 'message' => 'Group Deleted Successfully');
    } catch (Exception $e) {

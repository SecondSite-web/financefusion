...
$crm= new Crm($pdo);
$log = new Logger('Modify Ticket Form');
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    try {
        ...
        $nonceTest = $dashNonce->verifyNonce($_POST['nonce'], 'modify-ticket-form');
        if ($nonceTest === false) {
        ...
        $filters = array(
            ...
            'priority'  => 'trim|sanitize_string|lower_case',
        ...
        $rules = array(
            ...
            'priority'  => 'required|alpha_numeric|max_len,25|min_len,1',
            'dest_group' => 'required|alpha_numeric|max_len,25|min_len,1',
            'status' => 'required|alpha_numeric|max_len,25|min_len,1',
            'nonce'   => 'required'
        );
        $validator = new GUMP();
        ...
        $okMessage = 'Your ticket has been successfully modified!';

        $result = $crm->modifyTicket($_POST);
        if ($result === false) {
